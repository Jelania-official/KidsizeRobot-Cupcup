...
int main(int argc, char **argv)
{
    ...
    const char *tracePath = std::getenv("CUPCUP_MATCH_TRACE");
    std::ofstream trace;
    if (tracePath && *tracePath) {
        trace.open(tracePath);
        trace << std::setprecision(15);
        trace << "ros_time,sim_time,ball_x,ball_y,ball_z,ball_vx,ball_vy,ball_vz,"
                 "red_x,red_y,red_z,blue_x,blue_y,blue_z,ball_contacts,blue_contact\n";
        ball->enableContactPointsTracking(basicTime, true);
    }
    int ballMoveCnt = 0;
    ...
    while (super->step(basicTime) != -1 && rclcpp::ok()) {
        ...
        if (gameData.state == gameData.STATE_INIT) {
            ...
            if (trace) {
                const double *velocity = ball->getVelocity();
                const double *redPos = red_1->getPosition();
                const double *bluePos = blue_1->getPosition();
                int contacts = ball->getNumberOfContactPoints(true);
                bool blueContact = false;
                for (int contact = 0; contact < contacts; ++contact) {
                    webots::Node *other = ball->getContactPointNode(contact);
                    for (webots::Node *node = other; node; node = node->getParentNode()) {
                        if (node->getId() == blue_1->getId()) { blueContact = true; break; }
                    }
                    if (blueContact) break;
                }
                trace << judgeNode->get_clock()->now().seconds() << ',' << super->getTime() << ','
                      << ballPos[0] << ',' << ballPos[1] << ',' << ballPos[2] << ','
                      << velocity[0] << ',' << velocity[1] << ',' << velocity[2] << ','
                      << redPos[0] << ',' << redPos[1] << ',' << redPos[2] << ','
                      << bluePos[0] << ',' << bluePos[1] << ',' << bluePos[2] << ','
                      << contacts << ',' << (blueContact ? 1 : 0) << '\n';
            }
            bool goal = false;
