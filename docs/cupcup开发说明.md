...
策略仅修改 `src/cupcup/src/player.cpp` 标记允许修改的区域。没有修改守门员、裁判、消息定义、动作参数或运动引擎。使用 OpenCV C++视觉和随提交提供的模型，不需要 GPU。
...
2. 比赛控制器：`ros2 run gamectrl gamectrl`，选择 cupcup 为红方、goalkeeper 为蓝方。
3. 我方策略：`ros2 launch cupcup player_launch.py`
...
```bash
ros2 run cupcup cupcup cupcup_1 --ros-args \
